  ...
  mutations: {
        ...
        axios.get(state.strapi + '/players?_limit=-1').then((ps) => {
          ...
          state.players.forEach((p) => {
            const arr_prs = prs.filter((pr) => {
              if (pr.player) return pr.player.id == p.id;
              return false;
            });
            p.p = arr_prs.length;
      ...
      axios.get(state.strapi + '/heroes?_limit=-1').then(({ data }) => {
        ...
        if (this.state.heroes.length === 0) {
          var heroes = [
            ...
            new Hero('npc_dota_hero_hoodwink', 'Hoodwink'),
            new Hero('npc_dota_hero_dawnbreaker', 'Dawnbreaker'),
            new Hero('npc_dota_hero_marci', 'Marci'),
          ];
          var n = 0;
          heroes.forEach((h) => {
            ...
            axios.post(this.state.strapi + '/heroes', h).then(() => {
              ...
              if (n == heroes.length) {
                axios
                  .get(state.strapi + '/heroes?_limit=-1')
                  .then(({ data }) => {
                    state.heroes = data.sort((a, b) =>
                      a.displayName.localeCompare(b.displayName)
                    );
    ...
    [INIT_TOURNAMENTS](state) {
      axios.get(state.strapi + '/tournaments?_limit=-1').then(({ data }) => {
        state.tournaments = data.sort((a, b) =>
    ...
    [INIT_TOURNAMENT_TYPE](state) {
      axios.get(state.strapi + '/tournament-types').then(({ data }) => {
        ...
        if (data.length === 0) {
          ALL.forEach((tt) => {
            axios
              .post(state.strapi + '/tournament-types', new TournamentType(tt))
              .then(({ data }) => {
                state.tournamentTypes.push(data);
                state.tournamentTypes.sort((a, b) =>
